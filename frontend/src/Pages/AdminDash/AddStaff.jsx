...
const AddStaff = () => {
  ...
  const handleAdd = async () => {
    if (validateForm()) {
      try {
        await axios
          .post(`${configs.apiUrl}/staff/staff`, newObject, {
  ...
  const handleEdit = async () => {
    if (validateForm()) {
      ...
      try {
        await axios.put(`${configs.apiUrl}/staff/staff/${id}`, newObject, {
          headers: { Authorization: `Bearer ${token}` },
        });
        Swal.fire({
          title: 'Success!',
  ...
  return (
    ...
    >
      <AppBar
        position="fixed"
        style={{ backgroundColor: '#1c2331', boxShadow: 'none' }}
      >
        <Toolbar>
          {info.editBtn ? (
            <Typography
          ...
          <Grid container spacing={3}>
            ...
            <Grid item xs={12}>
              <TextField
                ...
                onChange={(e) =>
                  setNewObject({ ...newObject, phone: e.target.value })
                }
                error={!!errors.phone}
